...
const App = () => {
  ...
  const handleEvidenceUpload = async (e) => {
    ...
    try {
      ...
      fetchStats();
      if(res.data.result === 'pass') addLog('VALIDATOR', `Γ£ô PASSED block #${mId}.`);
      else addLog('RISK', `Γ£ù FAILED block #${mId}. Evidence insufficient.`);
    } catch (error) {
  ...
  const handleKbUpload = async () => {
    ...
    try {
      ...
      addLog('KB', `Γ£ô "${res.data.title}" indexed. AI summary generated.`);
      fetchKbDocs();
      fetchStats();
  ...
  const formatCurrencyCompact = (val) => {
    if(val >= 10000000) return 'Γé╣' + (val / 10000000).toFixed(1) + ' Cr';
    if(val >= 100000) return 'Γé╣' + (val / 100000).toFixed(1) + ' L';
    if(val >= 1000000) return '$' + (val / 1000000).toFixed(1) + 'M';
    if(val >= 1000) return '$' + (val / 1000).toFixed(0) + 'K';
    if(val === 0) return 'Γé╣0';
    return '$' + val;
  };
    ...
    <div className="app-container">
      ...
      <main className="main-area">
        ...
        <div className="dashboard-content">
          ...
          {/* ΓòÉΓòÉΓòÉ DASHBOARD VIEW ΓòÉΓòÉΓòÉ */}
            ...
            <>
              ...
              <div className="top-widgets">
                ...
                <div className="widget-card">
                  ...
                  <div className="penalty-value" style={{color: penaltyTickerVal === 0 ? 'var(--success-green)' : '#ff9d9d'}}>
                    {penaltyTickerVal === 0 ? 'Γé╣0 Γ£ô' : formatCurrencyCompact(penaltyTickerVal)}
                  </div>
                  <div className="penalty-trend"><Activity size={14} /> {penaltyTickerVal === 0 ? 'All compliant ΓÇö no exposure' : 'Validate evidence to reduce exposure'}</div>
              ...
              {loading && (
                <div style={{padding: '2rem', textAlign: 'center', color: 'var(--primary-amber)'}}>
                  ...
                  <p style={{fontFamily: 'var(--font-data)'}}>PIPELINE ACTIVE ΓÇö GAP ANALYSIS IN PROGRESS...</p>
                </div>
              )}
                ...
                <div className="bottom-split">
                  ...
                  <div className="kanban-board">
                    <div className="kanban-header">
                      <span>Active Directives {result?.regulation ? `ΓÇö ${result.regulation}` : ''}</span>
                      {/* Gap summary chips */}
                      {maps.length > 0 && (
                        <div style={{display: 'flex', gap: '0.5rem', fontSize: '0.7rem', fontFamily: 'var(--font-data)'}}>
                          <span style={{background: 'rgba(34,197,94,0.15)', color: 'var(--success-green)', padding: '2px 6px', borderRadius: '2px'}}>Γ£ô COMPLIANT: {alreadyCompliant.length}</span>
                          <span style={{background: 'rgba(234,179,8,0.15)', color: 'var(--primary-amber)', padding: '2px 6px', borderRadius: '2px'}}>ΓÜá GAPS: {inProgress.length}</span>
                          <span style={{background: 'rgba(99,102,241,0.15)', color: '#a5b4fc', padding: '2px 6px', borderRadius: '2px'}}>Γ£ô VALIDATED: {validated.length}</span>
                        </div>
                      )}
                    </div>
                        ...
                        {alreadyCompliant.map((m) => (
                          <div key={m.id} className="k-card" style={{borderLeftColor: 'var(--success-green)', opacity: 0.7}}>
                            ...
                            {m.matched_document && m.matched_document !== 'None' && (
                              <div style={{fontSize: '0.7rem', color: 'var(--success-green)', marginTop: '0.25rem'}}>Γ£ô Matched: {m.matched_document}</div>
                            )}
                          </div>
                        ))}
          ...
          {/* ΓòÉΓòÉΓòÉ KNOWLEDGE BASE VIEW ΓòÉΓòÉΓòÉ */}
                      ...
                      <div style={{display: 'flex', gap: '1rem'}}>
                        <div style={{flex: 1}}>
                          ...
                          <select value={kbDept} onChange={e => setKbDept(e.target.value)}
                            ...
                            <option value="__other__">Γ£Å∩╕Å Other (type below)</option>
                          </select>
                          {kbDept === '__other__' && (
